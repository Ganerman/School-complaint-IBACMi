-- Require documented repair completion and send role-specific alerts.
...
language plpgsql security definer set search_path = '' as $$
declare number text; complaint_title text;
begin
 ...
 returning complaint_number,title into number,complaint_title;
 insert into public.notifications(user_id,title,message,notification_type,reference_id)
 values(new.staff_id,'New maintenance assignment',number||': '||complaint_title||'. Open the complaint to view its location and instructions.','assignment',new.complaint_id);
 return new;
end $$;
