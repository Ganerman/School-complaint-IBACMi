...
export function DashboardPage(){
 const{profile}=useAuth();const nav=useNavigate();const[items,setItems]=useState<Complaint[]>([]);const[loading,setLoading]=useState(true)
 const load=useCallback(async()=>{const{data}=await complaintService.list();setItems((data||[]) as unknown as Complaint[]);setLoading(false)},[])
