...
export function NewComplaintPage() {
  ...
  useEffect(() => {
    void Promise.all([
      complaintService.categories(),
      complaintService.locations(),
  ...
  async function submit(event: FormEvent) {
    ...
    if (error) {
      ...
      let constraintMessage =
        "One of the complaint fields does not satisfy the database requirements.";
      ...
      const messages: Record<string, string> = {
        "42501":
          "Your account does not have permission to submit complaints. Check the profiles role and RLS policies.",
        "23502":
          "The complaint-number database trigger is missing or inactive. Apply the latest Supabase migration.",
        "23514": constraintMessage,
        "23503":
          "The selected category or location no longer exists. Please refresh the page.",
      ...
      return toast.error(
        messages[error.code] ||
          friendlyError(
            error,
            `Unable to submit complaint (code: ${error.code || "unknown"}).`,
          ),
        { duration: 8000 },
      );
    }
    if (file) {
      ...
      if (upload.error)
        toast.warning("Complaint saved, but the photo could not be uploaded.");
    }
    ...
    <div className="mx-auto max-w-3xl">
      ...
      <form className="card mt-7 grid gap-5 p-6 md:p-8" onSubmit={submit}>
        ...
        <div>
          ...
          {preview ? (
            <div className="relative mt-2 overflow-hidden rounded-xl border bg-slate-100">
              <img
                src={preview}
              ...
              <button
                type="button"
                className="absolute right-3 top-3 rounded-full bg-slate-900/70 p-2 text-white"
                onClick={() => {
            ...
            <label className="mt-2 grid cursor-pointer place-items-center rounded-xl border-2 border-dashed bg-slate-50 px-5 py-10 text-center hover:border-forest-400">
              <ImagePlus className="text-forest-600" />
              <b className="mt-2 text-sm">Choose a clear photo</b>
              <small className="mt-1 text-slate-400">JPEG, PNG, or WebP</small>
              <input
                className="hidden"
                type="file"
